...
final currentPlanProvider =
    FutureProvider.autoDispose<SubscriberPlanResponse>((ref) async {
  return ref.watch(homeRepositoryProvider).fetchPlanList();
});

final sessionListProvider =
    FutureProvider.autoDispose<SessionListResponse>((ref) async {
...
class HomeRepository {
  ...
  Future<SubscriberPlanResponse> fetchPlanList() async {
    ...
    if (data is Map<String, dynamic>) {
      ...
      if (returnCode != null && returnCode != 0) {
        throw Exception(data['returnMessage']?.toString() ?? 'Failed to load plans');
      }
  ...
  Future<SessionListResponse> fetchSessionList() async {
    ...
    if (data is Map<String, dynamic>) {
      ...
      if (returnCode != null && returnCode != 0) {
        throw Exception(data['ReturnMessage']?.toString() ?? 'Failed to load session data');
      }
